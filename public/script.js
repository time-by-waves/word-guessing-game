class WordGuessingGame {
  ...
  setupSocketListeners() {
    ...
    this.socket.on('gameWon', (data) => {
      ...
      this.showMessage(`Game Over! The word was "${data.targetWord}"`, 'success');
  ...
  showGameMenu() {
    ...
    this.usernameDisplay.textContent = this.currentPlayer.displayName || this.currentPlayer.username;

    if (!this.isGuest) {
      this.loadPlayerStats();
    } else {
      document.getElementById('playerStats').innerHTML = '<p>Playing as guest</p>';
    }
  }

  async loadPlayerStats() {
    try {
      const response = await fetch(`/api/players/${this.currentPlayer.id}/stats`);
      if (response.ok) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.isComplete && data.targetWord) {
          this.targetWord = data.targetWord;
          this.showMessage(`Game is complete. The word was: ${data.targetWord}`, 'success');
          this.submitBtn.disabled = true;
          this.guessInput.disabled = true;
        }
      }
  ...
  toggleSortOrder() {
    ...
    if (this.sortByCloseness) {
      ...
      this.historyTitle.textContent = 'Guess History (By Alphabetical Closeness)';
    } else {
      this.sortToggle.textContent = 'Sort by Closeness';
      this.historyTitle.textContent = 'Guess History (Newest First)';
    }
  ...
  updateGuessList(guesses) {
    ...
    sortedGuesses.forEach(guess => {
      ...
      guessElement.className = 'guess-item' + (guess.isCorrect ? ' correct' : '');

      const time = new Date(guess.timestamp || guess.guessed_at).toLocaleTimeString();
