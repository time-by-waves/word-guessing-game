...
class WordGameMCPServer {
  ...
  setupHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'test_game_flow',
          description: 'Test a complete game flow with automated player actions',
          inputSchema: {
            type: 'object',
            properties: {
              playerCount: {
  ...
  async testGameFlow(args) {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        // Make some guesses
        const guesses = ['apple', 'banana', 'cherry', 'dragon', targetWord || 'example'];
  ...
  async analyzePerformance(args) {
    const { duration = 30, playerCount = 10 } = args;
