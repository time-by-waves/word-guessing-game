...
test.describe('Word Guessing Game E2E Tests', () => {
  ...
  test('should toggle sort order', async ({ page }) => {
    ...
    await expect(page.locator('#historyTitle')).toContainText(
      'By Alphabetical Closeness'
    );
  ...
  test('should be responsive on mobile', async ({ page }) => {
    ...
    const computedStyle = await inputSection.evaluate(
  ...
  test('should handle game completion', async ({ page }) => {
    ...
    const successMessageExists =
      (await page.locator('.message.success').count()) >= 0;
